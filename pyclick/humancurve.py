...
class HumanCurve():
  ...
  def generateCurve(self, **kwargs):
    ...
    """
    offsetBoundaryX = kwargs.get("offsetBoundaryX", 20)
    offsetBoundaryY = kwargs.get("offsetBoundaryY", 20)
    leftBoundary = kwargs.get("leftBoundary", min(
        self.fromPoint[0], self.toPoint[0])) - offsetBoundaryX
    rightBoundary = kwargs.get("rightBoundary", max(
        self.fromPoint[0], self.toPoint[0])) + offsetBoundaryX
    downBoundary = kwargs.get("downBoundary", min(
        self.fromPoint[1], self.toPoint[1])) - offsetBoundaryY
    upBoundary = kwargs.get("upBoundary", max(
        self.fromPoint[1], self.toPoint[1])) + offsetBoundaryY
    knotsCount = kwargs.get("knotsCount", 0)
    knotsCount = random.randint(1, 10)
    if knotsCount <= 7:
      knotsCount = 1
    # elif knotsCount <= 9:
      # knotsCount = 2
    ...
    distortionFrequency = kwargs.get("distortionFrequency", 0.2)
  ...
  def generateInternalKnots(self,
    ...
    knotsX = np.random.choice(
        range(leftBoundary, rightBoundary), size=knotsCount)
  ...
  def distortPoints(self, points, distortionMean, distortionStdev, distortionFrequency):
    ...
    """
    if not(isNumeric(distortionMean) and isNumeric(distortionStdev)
